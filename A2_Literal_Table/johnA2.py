    ...
    ********************************************************************/
    ...
    def insert(self, literal_data):
        ...
        if self.head is None:
            ...
            self.log_entries.append(f"Inserted literal {literal_data.name} as head.")
        else:
            ...
            while current.next is not None:
                ...
                if current.literal_data.name == literal_data.name:
                    self.log_entries.append(f"Duplicate literal found: {literal_data.name}.")
                    return
                current = current.next
            
            # Insert at the end of the list
            current.next = LiteralNode(literal_data)
            self.log_entries.append(f"Inserted literal {literal_data.name}.")
    ...
    def update_addresses(self, start_address=0):
        """
        Assign addresses to literals sequentially.
        ...
        self.log_entries.append(f"Addresses updated starting from {start_address}.")
